...
import experimental.complete.CompleteMatcher
import experimental.complete.TransformationHandler
import experimental.complete.CompleteQueries
...
class Transformation {
	...
	val extension CompleteQueries queries = CompleteQueries.instance
	...
	private def createTransformation() {
		...
		fixedPriorityResolver.setPriority(completeRule.ruleSpecification, 1)

		transformation = EventDrivenTransformation.forEngine(engine).setConflictResolver(fixedPriorityResolver).addRule(
			completeRule).build
	}

	val completeRule = createRule.precondition(CompleteMatcher.querySpecification).action(CRUDActivationStateEnum.CREATED) [
		handler.completeMatch(sourceParent, sourceEntry, targetParent, targetEntry, correspondence, identifier)
